...
def udpserver_cam1(queue):

    global c2w
    cam_id = 1

    port_nb = 3001 # cam #1 --> 3001 | cam #2 --> 3002 | cam #3 --> 3000
...
def udpserver_cam2(queue):

    global c2w
    cam_id = 2

    port_nb = 3002 # cam #1 --> 3001 | cam #2 --> 3002 | cam #3 --> 3000
    server_addr = ('172.16.222.31', port_nb)
    ssock = socket.socket(socket.AF_INET, socket.SOCK_STREAM)
    print("Socket created")
...
def udpserver_cam3(queue):

    global c2w
    cam_id = 3

    port_nb = 3000 # cam #1 --> 3001 | cam #2 --> 3002 | cam #3 --> 3000
    server_addr = ('172.16.222.31', port_nb)
    ssock = socket.socket(socket.AF_INET, socket.SOCK_STREAM)
    print("Socket created")
...
def use_xyz(queue):
    ...
    try:
        ...
        while(True):
            ...
            payload_out = Payload(cam_id, pixels[0], pixels[1])
            nsent = s.send(payload_out)
...
if __name__ == "__main__":
    ...
    cam_1 = multiprocessing.Process(target=udpserver_cam1, args=(queue,))
    cam_2 = multiprocessing.Process(target=udpserver_cam2, args=(queue,))
    cam_3 = multiprocessing.Process(target=udpserver_cam3, args=(queue,))
